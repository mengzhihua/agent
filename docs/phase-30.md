...
会话 continue / rewind / 手动 compact 见 [期 31](phase-31.md)。累计 token 用量见 [期 32](phase-32.md)。

不做：Apple 公证 / Developer ID 证书（未公证仍可能要 `xattr -cr agent`）、OAuth、图片 prompt、Docker workspace / 云端 VM、Playwright、npm 公有源。
