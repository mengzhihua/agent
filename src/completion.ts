...
_agent() {
  ...
  local cmds="acp eval session memory doctor update uninstall completion config init login logout --help --version --print --quiet --output-format --yes --plan --workspace --resume --list --file --model --provider --sandbox --browser --no-mcp"
...
function powershellCompletion(): string {
  ...
  $cmds = @(
    'acp','eval','session','memory','doctor','update','uninstall','completion','config','init','login','logout',
